...
exports.Narrative = function (narrator, audience) {
    ...
    function say(event) {
        ...
        if (event.verb.conjugatePredicate) {
            parts.push(event.verb.conjugatePredicate(event, self));
        } else {
            parts.push(verb(event.verb, event.subject));
            if (event.verb.conjugateObject) {
                var conjugateObject = event.verb.conjugateObject(event, self);
                conjugateObject && parts.push(conjugateObject);
